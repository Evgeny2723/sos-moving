...
function addInputPhoneMask() {
    ...
    e.forEach(e => {
        ...
        e.addEventListener("focusout", l, !1),
        e.addEventListener("beforeinput", n, !1)
    }
    )
}
...
function removeErrorClassOnInput(e) {
    ...
    "radio" == e.type && e.addEventListener("click", () => {
        document.querySelectorAll(`[name=${e.dataset.name}]`).length && document.querySelectorAll(`[name=${e.dataset.name}]`).forEach(e => {
            e.classList.remove("is-error")
        }
        )
    }
...
function formValidation(e) {
    ...
    return s.forEach(e => {
        ...
        e.classList.remove("is-error"),
        e.classList.contains("is-phone") && !e.classList.contains("valid") && (e.classList.add("is-error"),
        t = !1),
        "true" === a.required && (s || (e.classList.add("is-error"),
        t = !1),
        "radio" == e.type && !document.querySelector(`[name=${e.dataset.name}]:checked`) && (document.querySelectorAll(`[name=${e.dataset.name}]`).forEach(e => {
...
Webflow.push(function() {
    ...
    $(".is-have-slider").each(function(e) {
        ...
        slidersArr[e].slider.slick({
            ...
            responsive: [{
                ...
                breakpoint: 768,
                settings: {
                    variableWidth: slidersArr[e].variableWidthMobile,
                    slidesToShow: slidersArr[e].slidesToShowMobile
                }
            }, ]
        })
    })),
    ...
    $(".w-page-count").each(function() {
        ...
        for (let l = r; l <= o; l++) {
            let n = l
              ...
              , p = n == a
              , u = $("<a>", {
                class: [p && "is-current", "page-link"].filter(e => e).join(" "),
                href: c,
                text: n
            });
            $(this).append(u)
    ...
    $(".article-content-area h2").each(function(e) {
        let t = $(".article-toc-list")
          , s = 97 + e
          , a = $(this).html();
        a.indexOf(". ") + 1 && (a = a.split(". ")[1]),
        $(this).attr("id", String.fromCharCode(s)),
        t.append('<li class="article-toc-item"><a href="#' + String.fromCharCode(s) + '" class="article-toc-link">' + a + "</a></li>")
    })
}),
addInputPhoneMask();
var forms = document.querySelectorAll("form");
forms.length && forms.forEach(e => {
    checkValidationFormOnSubmit(`#${e.getAttribute("id")}`),
...
$.fn.select2.amd.define("select2/data/googleAutocompleteAdapter", ["select2/data/array", "select2/utils"], function(e, t) {
    ...
    s.prototype.query = function(e, t) {
        var s = function(e, s) {
            ...
            if (s != google.maps.places.PlacesServiceStatus.OK && t(a),
            e.length)
                for (var i = 0; i < e.length; i++)
                    a.results.push({
                        id: e[i].place_id.toString(),
                        text: e[i].description.toString()
                    });
            a.results.push({
                id: " ",
...
function getDetails(e, t) {
    (new google.maps.Geocoder).geocode({
        ...
        if ("OK" === s) {
            ...
            window.alert("Geocoder failed due to: " + s)
    })
}
...
$("form").length && ($("form").each(function() {
    $(this).hasClass("referer-form") || ($(this).attr("data-api-redirect", $(this).attr("redirect")),
    $(this).removeAttr("redirect data-redirect")),
    $(".services-hero-h1").length ? ($(this).attr("data-name", "Form in " + $(".services-hero-h1").html() + " Page."),
    $(this).attr("name", "Form in " + $(".services-hero-h1").html() + " Page.")) : ($(this).attr("data-name", "Form in " + $("title").text() + "."),
