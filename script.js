...
    	});
	...
	$("form").on("submit", function () {
        ...
        // --- НАЧАЛО: ЛОГИКА ЗАГЛУШЕК ---
        const emailRegex = /\S+@\S+\.\S+/;

        if (!t.data.field_first_name) t.data.field_first_name = "Not Provided";
        // Заменяем старую заглушку "n/a" на новую для единообразия
		...
		if (!e.hasClass("referer-form")) {
			...
			(t = JSON.stringify(t)), console.log("Отправляемые данные:", t);
			...
			$.ajax({
				...
				statusCode: {
					400: function (e) {
						...
						l.html(t.status_message || "Bad Request"), l.show(), a.val(i);
					},
				},
				success: function (e) {
					var t = JSON.parse(e);
				...
				error: function (e, t, s) {
					l.html("Ошибка отправки. Проверьте консоль."), l.show(), a.val(i);
				},
			});
		}
	});
